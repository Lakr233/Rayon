...
public struct AES {
    ...
    public static let shared: AES = {
            ...
            let key = serialNumber + serialNumber + serialNumber
            guard let aes = AES(key: key, iv: key) else {
                UIBridge.presentError(with: "Failed to initialize crypto engine for Rayon")
                exit(EPERM)
            }
            return aes
        #else
            ...
            guard let key = key else {
                UIBridge.presentError(with: "Failed to load crypto keys for Rayon")
                exit(EPERM)
            }
            guard let aes = AES(key: key, iv: key) else {
                UIBridge.presentError(with: "Failed to initialize crypto engine for Rayon")
