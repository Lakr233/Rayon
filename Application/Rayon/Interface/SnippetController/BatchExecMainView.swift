...
struct BatchExecMainView: View {
    ...
    @State var totoalProgress: Double = 0
    ...
    var body: some View {
        progressView
            ...
            .animation(.interactiveSpring(), value: totoalProgress)
    ...
    var progressView: some View {
        Group {
            if currentProgress == 0, totoalProgress == 0 {
                ProgressView()
            } else if currentProgress < totoalProgress {
                VStack(alignment: .leading, spacing: 15) {
                    ProgressView()
                        .frame(maxWidth: .infinity)
                        .padding()
                    ProgressView(value: currentProgress, total: totoalProgress) {
                        Text("Operation is in progress: \(Int(exactly: totoalProgress - currentProgress) ?? 1) remain")
                    }
    ...
    func updateProgress() {
        ...
        totoalProgress = Double(exactly: context.machines.count) ?? 0
